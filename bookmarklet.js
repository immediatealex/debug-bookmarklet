...
<style>
#debug-tool-panel { position:fixed; top:50%; left:50%; transform:translate(-50%, -50%); z-index:9999999; padding:30px; background:black; color:white; box-shadow:0 10px 20px #0007; border:1px solid silver; font-size:10px; line-height:14px; font-family:arial; }
...
const h = {}; res.headers.forEach((v,k) => h[k] = v);
const getPoweredBy = () => {
  if (h['x-powered-by']) return h['x-service-name'] ? `${h['x-powered-by']}<br /><small>(${h['x-service-name']})</small>` : h['x-powered-by'];
